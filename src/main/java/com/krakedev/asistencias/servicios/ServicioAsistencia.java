...
public class ServicioAsistencia {
	...
	public ArrayList<Asistencia> consultarAsistencia(String cedula) {
		ArrayList<Asistencia> asistenciasEncontradas = new ArrayList<Asistencia>();

		for (RegistroAsistencia registro : registros) {
			Estudiante estudiante = registro.getEstudiante();

			if (estudiante.getCedula().equals(cedula)) {
				asistenciasEncontradas.add(registro.getAsistencia());
			}
		}
	...
	public ArrayList<RegistroAsistencia> listarRegistros() {
		return registros;
	}
}